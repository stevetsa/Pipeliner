...
sampletable=pd.read_csv(sys.argv[2],header=0,sep="\t")
sampletable['sampleName']=sampletable['sampleName'].map(lambda x:x.split("/")[-1].split(".")[0])

#outdir=sys.argv[1]
igfiles=sys.argv[1].split()
contrasts=sys.argv[3].split()
rsemref=sys.argv[4]
itype=sys.argv[5]

annotate=pd.read_csv(sys.argv[6],header=None,sep=" ",usecols=[0,2])
...
for i in range(0,len(contrasts),2):
    ...
    cmfile= ".".join([pair,itype,"counts","matrix"])
    ebseqfile=".".join([pair,itype,"EBSeq"])
    files=list()
...
finaloutfile=open("EBSeq_{}_completed.txt".format(itype),'w')
